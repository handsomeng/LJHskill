...
从一门约 20 万字的内容电商实操课里，把可执行的部分（诊断表、判断标准、SOP、填空模板）提炼成 13 个 Agent skill，覆盖选品、定位、卖点、场景、内容、验证、投放、算账全链路。课程正文不在本仓库，这里只有拿来就能跑的工具。

**最新版本 v0.2.1**

- **v0.2.1**：重写 13 个工具收尾时的那句找人话术，一条一个说法，去掉模板腔。

- **v0.2.0**：README 重构，加新手三步走、四个使用场景和作者介绍。

- **v0.1.4**：全量话术过了一遍 AI 味体检（22 条特征扫描），清掉播报腔、总结升华句和黑话，示例故事补了真实细节。

- **v0.1.3**：千川诊断树补了三个缺口：开播时长场次检查、审核和链接状态检查、外因兜底节点；结论支持多根因并列。

- **v0.1.2**：主入口路由表重写成商家的症状语言（「被骂智商税」「跑量猛但不下单」这类原话直接能路由），加了「值不值得」消歧判据和「管不了」边界清单。

- **v0.1.1**：修复安装只装到主入口、拿不到其余 12 个工具的问题。

- **v0.1.0**：首发，13 个工具全部上线。
...
**场景一，新手入局**：手上有个品，还没想清楚该不该做。从 `/ljh-xuanpin` 开始判断值不值得打，走完定位、卖点、场景、验证、内容、投放，一路走到 `/ljh-suanzhang` 把账算清楚。13 个工具串起来就是一条完整的决策链。

**场景二，已经在跑的盘子**：品在跑了，但哪里疼就查哪里。掉量了用 `/ljh-qianchuan`，口碑被骂用 `/ljh-maidian`，脚本转化不理想用 `/ljh-jiaoben`。不用从头走一遍，13 个工具随时单独调用。

**场景三，只拿一个工具用**：不想装全套也可以。13 个 skill 每个都能单独安装、单独跑，互不依赖，拿走你需要的那一个就够。
