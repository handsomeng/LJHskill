...
**v0.1.1 更新**：修复了安装只装到主入口、拿不到其余 12 个工具的问题；重写了主入口的路由逻辑；对全部工具的话术做了一轮打磨。
...
**场景四，给团队用**：编导用 `/ljh-jiaoben` 评审脚本、用 `/ljh-yinzi` 拆内容因子；投手用 `/ljh-qianchuan` 排查千川问题；商务用 `/ljh-daren` 选号、用 `/ljh-liangye` 准备见主播的材料。团队装同一套，各自按自己的岗位取用。
