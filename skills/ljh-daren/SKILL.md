...
达人选号要判断三件事：触达是否真实、信任是否成立、人群是否匹配。任何一项失真都会造成预算损失。判断一个账号值不值得投，不能只看粉丝量和单条爆款，要看下面六个维度的组合结果。
...
3. 交付完成后，把本次关键结论追加进档案的「结论时间线」（日期 + 工具名 + 一两句结论），不覆盖旧内容；完整交付物另存到 `ljh-档案/交付物/日期_ljh-daren_主题.md`。中途没跑完的交互不写档案。
...
- 只有一条爆款数据、没有近 30/90 天整体播放数据：不能用单条爆款代表账号真实流量水平，传播力这一维标记为「待核实」，要求补近 30/90 天播放中位数后复核，不直接计入风险。
...
- 任一项低于 5%：如果点赞很多但对应的评论、分享或收藏很低，明显塌陷，直接标记风险信号，这是刷赞的典型特征。
- 任一项落在 5% 到 10% 之间：通过，数据健康。
- 任一项落在 10% 到 20% 之间：先判通过，但标注「偏高，需结合反作弊三查核实」，不要不加说明地当成完全健康的数据。
- 任一项超过 20%：标记风险信号，不是数字越高越好，可能是刷评论或刷收藏，需要结合反作弊三查里的假互动特征一起核对。
...
1. 假播放：播放高但评论收藏弱、挂车成交弱、观看画像异常，或播放粉丝比显著偏离类目常态 → 命中就标记；没有对应数据（比如没有观看画像、没有挂车成交数据）判不了的，标记「无法核查，需补 XX 数据」
2. 假粉：粉丝画像和观看画像撕裂过大 → 命中就标记；没有粉丝画像或观看画像其中一项的，标记「无法核查，需补 XX 数据」
3. 假互动：点赞高但评论模板化、收藏分享极低 → 命中就标记；没有具体评论内容或互动数据的，标记「无法核查，需补 XX 数据」

只要某一项标记了「无法核查」，报告里禁止把这一项写成「数据结构自然」，只能如实写「未核查，需补 XX 数据」，不能用「暂时看起来正常」这种模糊说法替代。

三项全部命中「未命中」：反作弊结论标记为"数据结构自然"。
任何一项命中「命中」：反作弊结论标记为"存在风险"，并说明命中了哪一项、具体数据依据是什么。
没有命中项，但有一项或多项是「无法核查」：反作弊结论标记为"部分无法核查"，列清楚是哪几项、各自需要补什么数据。
...
1. 反作弊三查里假播放、假粉中的任意一项是「真命中」（无法核查不算命中）→ **放弃**。这一条优先级最高，不管六维通过项数多少，只要真命中假播放或假粉，直接放弃。
2. 没有真命中假播放、假粉的前提下，再看六维通过项数：
   - 通过项数至少 5 项 → **可谈**
   - 通过项数 3 到 4 项 → **压价谈**（或反作弊三查命中 1 项轻微风险信号，如仅评论略模板化但其余数据健康，也归这一档）
   - 通过项数不超过 2 项，且待核实项数 ≥ 2 → **暂缓补数**：这种情况说明账号本身没有暴露真实风险信号，只是数据给得不够，不能直接判放弃。输出一份补数清单，逐项列清楚缺的是哪几项数据（比如带货力缺 GPM/CPM、人群缺近 30-90 天观看画像），请用户补齐后回来复核，复核时重新走一遍六维核查和反作弊三查。
   - 通过项数不超过 2 项，且待核实项数 ≤ 1（意味着大部分维度是拿到数据后判出来的明确风险，不是缺数据）→ **放弃**
...
- 活跃度：每周 3 条 → 通过
- 商业力：评赞率 = (300+150)/8000 ≈ 5.6%，落在 5% 到 10% 之间，数据健康；收藏率 = 900/8000 ≈ 11.3%，落在 10% 到 20% 之间，标注「偏高，需结合反作弊三查核实」→ 通过
...
- 把"评论多"直接等同于"互动好"，不看评论内容质量。评论多但全是模板化短句，仍属于假互动的一种，要按反作弊三查里的假互动标准重新判断。
...
选号诊断通过（可谈或压价谈）后，建议输入 `/ljh` 获取下一步推荐，或直接用 `/ljh-brief` 准备主播合作 Brief 和可选的当面沟通提词，把“选中”推进到“谈成”。
