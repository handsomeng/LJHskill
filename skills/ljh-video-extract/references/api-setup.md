...
## Apify 通道（抖音，已有凭证则跳过本节）

抖音作品的完整提取（数据 + 口播 + 无水印 mp4/mp3）优先走 Apify，已有 Apify token 时
零配置即可使用（下载按条小额计费，本地转写免费）。token 的存放与发现顺序参考
[apify-api.md](apify-api.md)「脚本的 token 发现顺序」；配置方式：
